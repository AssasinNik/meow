...
void professor_project::setData(const QString &projectName){
    ...
    if (!query.exec(QString("SELECT leadername, email FROM leaders WHERE leaderid NOT IN (SELECT leaderid FROM projectleaders WHERE projectid = %1)").arg(projectId))) {
...
void professor_project::on_pushButton_clicked()
{
    ...
    insertQuery.prepare("INSERT INTO ProjectLeaders (ProjectID, LeaderID) VALUES (:projectID, :leaderID)");
