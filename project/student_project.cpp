...
void student_project::setData(const QString &projectName){
    ...
    if (!query.exec(QString("SELECT studentname, email FROM students WHERE student_id_card NOT IN (SELECT student_id_card FROM projectstudents WHERE projectid = %1)").arg(projectId))) {
...
void student_project::on_pushButton_clicked()
{
    ...
    insertQuery.prepare("INSERT INTO ProjectStudents (ProjectID, Student_ID_Card) VALUES (:projectID, :studentID)");
